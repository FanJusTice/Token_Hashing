...
void level_expand(level_hash *level)
{
    ...
    uint64_t old_idx; // Rehash all data from old bottom level to new level
    for (old_idx = 0; old_idx < pow(2, level->level_size - 1); old_idx ++) {
        uint64_t i, j;
        for(i = 0; i < ASSOC_NUM; i ++){
            if (level->buckets[1][old_idx].token[i] >= VAILD_DATA)
            {
    ...
    uint64_t new_idx; // Rehash queried data from new bottom level to new level
    for (new_idx = 0; new_idx < pow(2, level->level_size - 1); new_idx ++) {
        uint64_t i, j;
        for(i = 0; i < ASSOC_NUM; i ++){
            if (level->buckets[1][new_idx].token[i] >= QUERY_ONCE)
            {
    ...
    if(level->level_item_num[0] > level->level_item_num[1]){
        ...
        for(i = 2; i > 0; i --){
            for(j = 0; j < ASSOC_NUM; j ++){
                ...
                {
                    level->buckets[i-1][f_idx].token[j]++;
            ...
            for(j = 0; j < ASSOC_NUM; j ++){
                ...
                {
                    level->buckets[i-1][s_idx].token[j]++;
...
uint8_t level_update(level_hash *level, uint8_t *key, uint8_t *new_value)
{
    ...
    for(i = 0; i < 2; i ++){
        for(j = 0; j < ASSOC_NUM; j ++){
            if (level->buckets[i][f_idx].token[j] >= 1&&strcmp(level->buckets[i][f_idx].slot[j].key, key) == 0)
            {
                level->buckets[i][f_idx].token[j] = VAILD_DATA;
        ...
        for(j = 0; j < ASSOC_NUM; j ++){
            if (level->buckets[i][s_idx].token[j] >= 1&&strcmp(level->buckets[i][s_idx].slot[j].key, key) == 0)
            {
                level->buckets[i][s_idx].token[j] = VAILD_DATA;
    ...
    for(i = 0; i < ASSOC_NUM; i ++){
        ...
        for(j = 0; j < ASSOC_NUM; j ++){
            if (level->buckets[0][f_idx].token[j] == 0)
            {
                ...
                level->buckets[0][f_idx].token[j] = level->buckets[1][idx].token[i];
                level->buckets[1][idx].token[i] = EMPTY;
            ...
            else if (level->buckets[0][s_idx].token[j] == 0)
            {
                ...
                level->buckets[0][s_idx].token[j] = level->buckets[1][idx].token[i];
                level->buckets[1][idx].token[i] = EMPTY;
                level->level_item_num[0] ++;
                level->level_item_num[1] --;
                return i;
